      ...
      <div class="relative">
        ...
        <transition enter-active-class="transition duration-150 ease-out transform"
          ...
          <div v-show="dropdownOpen" class="absolute right-0 z-20 w-48 py-2 mt-2 bg-white rounded-md shadow-xl">
            <button class="block px-4 py-2 text-sm text-start w-full text-gray-700 hover:bg-indigo-600 hover:text-white"
              @click="goToProfile">
              Meus dados
            </button>
            <button class="block px-4 py-2 text-sm text-start w-full text-gray-700 hover:bg-indigo-600 hover:text-white"
            ...
            <ModalChangePassword v-if="openModal" @close="openModal = false" />
          </div>
        </transition>
      </div>
    </div>
