...
<template>
  <div class="flex justify-center space-x-2 my-5">
    ...
    <button v-if="currentPage > 1" :disabled="currentPage === 1" @click="changePage(currentPage - 1)"
      class="px-2 py-1 sm:px-4 cursor-pointer bg-white sm:py-2 mt-2 text-gray-600 border border-gray-600 rounded-lg hover:bg-blue-500 hover:border-blue-500 focus:outline-none">
    ...
    <button v-for="item in paginationItems" :key="item" @click.prevent="changePage(item)"
      class="px-2 py-1 sm:px-4 sm:py-2 mt-2  text-gray-600 border border-gray-600 rounded-lg hover:bg-blue-500 hover:border-blue-500 hover:text-white focus:outline-none"
      :class="{
      'px-3 py-1 border rounded': true,
      'bg-blue-500 text-white border-0': item === currentPage,
      'bg-white hover:border-blue-500': item !== currentPage,
    }">
      {{ item }}
    </button>
